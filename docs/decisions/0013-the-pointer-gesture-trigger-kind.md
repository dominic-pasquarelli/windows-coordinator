...
| **Guarantee statement** | Delivered as a discrete event carrying the capability id, the **matched zone token**, the **`GrantVersion`** it matched under ([ADR 0024](0024-grantversion-is-the-single-authoritative-version.md) — *this row first said "region-set version", which stopped being the right value once ADR 0021 split requested from granted*), and a tick delta. **Coalesced** under load, keyed on `(intent, zone token)`. **Not** guaranteed: one dispatch per physical detent, ordering against other input kinds, or delivery at all when the queue is saturated — a dropped cycle tick is a cosmetic loss, and pretending otherwise would mean buffering on the hook path |
